...
def next_card(totalnum, vtotal, c_hand):  
    ...
    #########OLD WAY CHAT SAID THE FOR LOOP BELOW IS BETTER W/ENUMERATE###########
    ...
    #        go_out.append(1)
    #        ii = a_hand.index(card) #the card's index in c_hand
    #        over.append(ii)
    #        #if all cards are over 31 we must GO!
    #        if sum(go_out) == len(a_hand):
    #            return None
    for aind, card in enumerate(a_hand):
        if current + card == 15 or current + card == 25 or current + card == 31:
            addval = len(totalnum) + 1     #how many cards have been played = points for the addition
            atup = (c_hand[aind], addval)
            options.append(atup)
    ...
    if len(options) > 1:
        ...
        for x in range(len(options)):
            if options[x][1] > points:
                ...
                pass
    #an option to score points is avaiable and ONLY 1 option
    elif len(options) == 1:
        play_card = options[0][0]
    #there are no options no score points and you have to GO
    elif len(options) == 0 and len(c_hand) == 0: #No cards available 
        return None 
    #NO options to score point BUT you can play a card 
    elif len(options) == 0:
        if len(over) != 0:
